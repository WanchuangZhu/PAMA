PAMA.PL=function(datfile,PLdatfile,nRe,iter=1000,init="EMM"){
  ...
  if(init=="EMM"){
    ...
    mallowsinfer=rank(rowMeans(dat))
    mallowsinfer[mallowsinfer>nRe]=0
    I.start=mallowsinfer
  }
  ...
  for(i in 1:iter){
    ...
    if (phi.new>0 & phi.new<1){
      #log.prob.start <- lapply(seq_len(ncol(Mallowsdat)), function(i) log(dmm(Mallowsdat[,i],I.start[I.start>0], -log(phi.start)*smlgamma.start[i])) )

      log.prob.start <- lapply(seq_len(ncol(Mallowsdat)), function(i) rankdist::DistancePair(Mallowsdat[,i],I.start[I.start>0])*(-log(phi.start)*smlgamma.start[i]) - logZ.MM((phi.start)*smlgamma.start[i],nRe))

      # log.prob.new <- lapply(seq_len(ncol(Mallowsdat)), function(i) log(dmm(Mallowsdat[,i],I.start[I.start>0], -log(phi.new)*smlgamma.start[i])) )
      log.prob.new <- lapply(seq_len(ncol(Mallowsdat)), function(i) rankdist::DistancePair(Mallowsdat[,i],I.start[I.start>0])*(-log(phi.new)*smlgamma.start[i]) - logZ.MM((phi.new)*smlgamma.start[i],nRe))
